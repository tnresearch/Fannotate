...
def classify_text(server,
                  ...
                  max_task_tokens,
                  seed,
                  output_dir=None):  # New parameter for specifying output directory
    """
    ...
    Args:
        ...
        seed (bool): Whether to set a seed or not.
    ...
    if output_dir:
        log_file = f"{output_dir}/classification_{model_name}_{prompt_name}.log"
        file_handler = logging.FileHandler(log_file)
        file_handler.setLevel(logging.DEBUG)
        logging.getLogger().addHandler(file_handler)

    # replications
    replications = list(range(0, replications))
    ...
    for replication in replications:
        ...
        for index, row in tqdm(bootstrap_sample.iterrows(), total=bootstrap_sample.shape[0], desc=f"Classifying with {model_name} and {prompt_name}"):
            ...
            # Truncate the tokens to the first max_task_tokens
            ...
            if torch.cuda.is_available():
                vram_usage = torch.cuda.memory_allocated() / 1024 / 1024 / 1024  # Convert to GB
